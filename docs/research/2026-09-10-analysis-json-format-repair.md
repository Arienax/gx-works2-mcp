...
状态：**confirmed**（本次实测的失败机制和纠正路径）；日期：2026-09-10。

同日后续按用户要求将工作台语言检查改为偏好，并加入实时进度与可选预览，见 [规格交互与实时进度](2026-09-10-spec-interaction-and-live-progress.md)。本文记录的是首次格式纠正验证时的策略。

## 复现证据

- 用户在本地 Web 工作台使用 `glm-5.3-flash` 分析“起保停”，历史任务两次以 `content:invalid_json_object` 失败。历史任务没有保存原始回复，不能断言它们的具体语法错误与本次完全相同（**unknown**）。
...
3. 纠正请求复用本次 Provider、输入上下文、语言及传输模式，给出解析错误位置，要求保持需求、设备、方案和问题，并返回完整分析对象。
