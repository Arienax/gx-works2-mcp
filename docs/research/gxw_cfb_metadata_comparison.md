...
基线：`main` 的 `4b78691`。此前“生成的 535B Program.pou 放入 native51 原生 allocation 后导线显示 PASS”及 dense-repack 导线不显示，来自已有实验结论，本次未重新打开 GX Works2 验证。
...
前面 stream `14` 的分配多出 4 个 mini-sector，恰好对应 native51 的 Program.pou 起点 31 与 dense-repack 起点 27 的差值。三个小流的额外占用合计 10 mini-sectors，恰好对应 root size 差额 640B。这里是分配数量的算术关系，不表示这些流或地址导致导线显示变化；本次不分析它们的内容。
