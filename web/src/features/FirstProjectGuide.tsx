...
const example = "FX3U 电机启停练习：X0 是瞬时启动按钮，X1 是停止信号（ON 时停止）。Y0 表示电机运行。按启动后保持运行，停止优先；上电不自动启动。请先整理 I/O、保持和复位规则，再让我确认规格。这是离线学习示例。";
export function FirstProjectGuide({t,hasSpec,disabled,onExample,onSpec,onGenerate}:{t:(s:string)=>string;hasSpec:boolean;disabled:boolean;onExample:(text:string)=>void;onSpec:()=>void;onGenerate:()=>void}) {
  ...
  return <div className="onboarding"><h2>{t("完成第一个工程")}</h2><p>{t("从一个电机启停练习开始，把需求、程序和测试联系起来。")}</p>
    <ol><li><strong>{t("说明输入和行为")}</strong><p>{t("输入是按钮或传感器，输出是希望控制的动作；同时说明停止、复位和上电时的状态。")}</p><Button disabled={disabled} onClick={()=>onExample(t(example))}>{t("填入启停示例")}</Button><p className="muted">{t("示例会填入 Agent 输入框，发送后开始需求分析。")}</p></li>
