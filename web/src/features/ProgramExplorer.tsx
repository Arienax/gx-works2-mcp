...
export function ProgramExplorer({pid,vid,proposalId,jobId,refreshKey,changeSummary,theme,selectedNetwork,initialAddress,onNetwork,t}:Props) {
  const [data,setData] = useState<Exploration|null>(null), [error,setError] = useState("");
  ...
  useEffect(()=>{let stale=false;setData(null);setError("");
    const path=proposalId?`/proposals/${proposalId}/explorer`:jobId?`/jobs/${jobId}/explorer`:`/projects/${pid}/versions/${vid}/explorer`;
    api<Exploration>(`${path}?theme=${theme}`).then(v=>{if(!stale){setData(v);setSelected(old=>selectedNetwork||(v.networks.some(n=>n.id===old)?old:v.networks[0]?.id)||"");}}).catch(e=>{if(!stale)setError(e.message);});
    return()=>{stale=true;};
  },[pid,vid,proposalId,jobId,theme,refreshKey]);
  useEffect(()=>{if(selectedNetwork){setSelected(selectedNetwork);targets.current[selectedNetwork]?.scrollIntoView({block:"nearest",behavior:"smooth"});}},[selectedNetwork,data]);
  useEffect(()=>{if(initialAddress){setAddress(initialAddress);setQuery(initialAddress);}},[initialAddress,data]);
  function select(id:string){setSelected(id);onNetwork(id);targets.current[id]?.scrollIntoView({block:"nearest",behavior:"smooth"});}
  if(error)return <p role="alert" className="panel-empty">{error}</p>;
  ...
  return <div className="program-explorer">
    ...
    <div className="explorer-main"><div className="explorer-canvas"><div className="explorer-drawing" style={{width:`${zoom*100}%`}}>
      <img src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(data.svg)}`} alt={t("可选择网络与地址的梯形图")} onError={()=>setError(t("梯形图加载失败，请点击“刷新结果 / 重绘梯形图”。"))} />
